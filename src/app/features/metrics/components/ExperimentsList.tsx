...
export default function ExperimentsList({ onSelectExperiment }: ExperimentsListProps) {
  ...
  if (isLoading) {
    return <div className="text-center py-8">Cargando experimentos...</div>
  }

  if (error) {
    return <div className="text-center py-8 text-red-600">Error al cargar experimentos</div>
  }
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold">Experimentos</h2>
        <button
      ...
      {!data || data.length === 0 ? (
        <div className="text-center py-8 text-gray-500">
          No hay experimentos. Crea uno para comenzar.
        </div>
      ) : (
